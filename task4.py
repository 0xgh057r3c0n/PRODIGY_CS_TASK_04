...
class KeyLoggerGUI:
    ...
    def show_terms_and_conditions(self) -> bool:
        ...
        accept_button = tk.Button(terms_window, text="Accept", command=lambda: self.accept_terms(terms_window), bg="#4caf50", fg="white")
        accept_button.pack(side=tk.LEFT, padx=10, pady=10)

        reject_button = tk.Button(terms_window, text="Reject", command=lambda: self.reject_terms(terms_window), bg="#f44336", fg="white")
        reject_button.pack(side=tk.RIGHT, padx=10, pady=10)

        terms_window.protocol("WM_DELETE_WINDOW", lambda: self.reject_terms(terms_window))

        self.root.wait_window(terms_window)
        return self.terms_accepted

    def accept_terms(self, terms_window):
        self.terms_accepted = True
        terms_window.destroy()

    def reject_terms(self, terms_window):
    ...
    def setup_main_window(self):
        self.root.title("Keylogger v" + self.VERSION)
        self.root.geometry("800x600")
        self.root.config(bg="#2b2b2b")

        banner_text = f"Keylogger by {self.AUTHOR} - Version {self.VERSION}"
        tk.Label(self.root, text=banner_text, font=("Arial", 16), fg="#ff9800", bg="#2b2b2b").pack(pady=10)
